...
export async function POST(request) {
  try {
    const { author_address, title, content } = await request.json();

    if (!author_address || !title || !content) {
      return NextResponse.json({ error: 'author_address, title, and content are required' }, { status: 400 });
    }

    await saveArticle(author_address, title, content);
